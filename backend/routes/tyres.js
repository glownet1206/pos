...
router.get('/stats/summary', async (req, res) => {
  try {
    ...
    if (biz === 'restaurant') {
      total = await db.getAsync(`SELECT COUNT(*) as total, 0 as totalStock FROM ${table}`);
      lowStock   = { count: 0 };
      outOfStock = await db.getAsync(`SELECT COUNT(*) as count FROM ${table} WHERE available = 0`);
    } else {
      total      = await db.getAsync(`SELECT COUNT(*) as total, COALESCE(SUM(stock),0) as totalStock FROM ${table}`);
